...
class ElapsedItem extends Bitrix24Entity
{
	...
	/*
	* example:
	* $B24ElapsedItem = new \Bitrix24\Task\ElapsedItem($this->arB24App);
	* $test = $B24ElapsedItem->add(38, array("SECONDS" => 60, "COMMENT_TEXT" => 'New 1111'));
	*/
	public function add($taskId, $fields)
	{
		$result = $this->client->call('task.elapseditem.add', array("TASKID" => $taskId, "ARFIELDS" => $fields));
		return $result;
	}
	...
	{
		$result = $this->client->call('task.elapseditem.update', array($taskId, $elapsedItemId, $fields));
		return $result;
	}
...
// -----------examples--------------------------------------------------------------------------------
                ...
                /*                
                $B24ElapsedItem = new \Bitrix24\Task\ElapsedItem($this->arB24App);
                $test = $B24ElapsedItem->add(38, array("SECONDS" => 3600, "COMMENT_TEXT" => 'New 1111'));
                \CB24Log::Add('add ElapsedItem :::'.print_r($test,true));
                */


/*                $B24ElapsedItem = new \Bitrix24\Task\ElapsedItem($this->arB24App);
                $timeTemp = $B24ElapsedItem->getList($artask[ID]);
                $artime = $timeTemp[result];
                \CB24Log::Add('artimeJSON :::'.print_r($artime,true));*/
// -------------------------------------------------------------------------------------------    